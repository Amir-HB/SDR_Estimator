...
plot(0.6, 0.5, xlab="p",
     ylab="error",
     xlim = c(min(range),max(range)), ylim = c(min(t_q2)-1 ,max(t_q2)+3),
     type = "n", main=paste("Error vs. dimension \n n=10p", Tex("$epsilon$"),"=0.2"), 
     cex.main=2.2, cex.lab=2.2, cex.axis=2)	
	for (i in 1:nrow(l_q2)){
		lines(x= range, y=t_q2[i,], type='o', col=cl[i], pch=pc[i], lwd = 2.5, cex=2)
		arrows(x0=range, y0=t_q1[i,], x1=range, y1=t_q3[i,], code=3, col=cl[i], lwd=2, length=0)
	}
